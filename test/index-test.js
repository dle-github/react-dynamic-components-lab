...
describe('BlogPost', () => {
	...
	it('passes the appropriate text as props to each comment component', () => {
		expect(comments.at(0).props().commentText).to.equal(
		...
		expect(comments.at(1).props().commentText).to.equal(
			'I am no longer accepting the things I cannot change. I am changing the things I cannot accept. - Angela Davis'
		);
		expect(comments.at(2).props().commentText).to.equal(
...
describe('ColorBox', () => {
	...
	it('correctly reduces the opacity by an additional 0.1 after the second recursive call', () => {
		...
		expect(boxTree.find(ColorBox).get(2).props.opacity).to.equal(
			0.8
		);
	});
